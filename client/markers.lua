...
RegisterNetEvent('voidrp-base:registerMarker')
AddEventHandler('voidrp-base:registerMarker', function(marker)
    if marker.coords == nil then
        print('Marker için kordinata ihtiyacın var!')
        return
    end
    ...
    if marker.command then
        RegisterCommand(marker.command.key, function(src, args, raw)
            ...
            if raw == command then
                TriggerEvent('voidrp-base:triggerCurrentMarkerAction')
            end
        end)
    end
...
RegisterNetEvent('voidrp-base:removeMarker')
AddEventHandler('voidrp-base:removeMarker', function(name)
    markers[name] = nil
end)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if isInMarker and not HasAlreadyEnteredMarker then
            ...
            TriggerEvent('voidrp-base:hasEnteredMarker', lastMarker)
        end
        if not hasExited and not isInMarker and HasAlreadyEnteredMarker then
            HasAlreadyEnteredMarker = false
            TriggerEvent('voidrp-base:hasExitedMarker')
        end
    end

end)

AddEventHandler('voidrp-base:hasExitedMarker', function()
    CurrentMarker = nil
    ESX.UI.Menu.CloseAll()
end)

AddEventHandler('voidrp-base:hasEnteredMarker', function(marker)
...
RegisterNetEvent('voidrp-base:triggerCurrentMarkerAction')
AddEventHandler('voidrp-base:triggerCurrentMarkerAction', function()
    if CurrentMarker and CurrentMarker.action ~= nil then
        CurrentMarker.action(CurrentMarker)
    end
end)

RegisterNetEvent('voidrp-base:registerBlip')
AddEventHandler('voidrp-base:registerBlip', function(blip)
    ...
    if blip.scale then
        SetBlipScale(_blip, getOrElse(blip.scale, 0.2))
    end
...
RegisterNetEvent('voidrp-base:updateBlip')
AddEventHandler('voidrp-base:updateBlip', function(blip, debug)
    ...
    if blip.coords then
        ...
        if _blip and GetBlipCoords(_blip) ~= blip.coords then
            ...
            TriggerEvent('voidrp-base:registerBlip', tempBlip)
            return
        end
